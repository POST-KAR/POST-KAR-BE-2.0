...
public class MarkerController {
        ...
        @Operation(summary = "Get Current Version", description = "Get the current marker database version info. App uses this to check for updates.\n\n"
                        +
                        "Response includes:\n" +
        ...
        @Operation(summary = "Get All Active Markers", description = "Get paginated list of all active markers with metadata.\n\n"
                        +
        ...
        @Operation(summary = "Get Markers Updated Since", description = "Get list of markers updated after a specific timestamp.\n\n"
                        +
        ...
        @Operation(summary = "Get Single Marker", description = "Get detailed information for a specific marker by its markerId.\n\n"
                        +
        ...
        public ResponseEntity<Marker> getMarker(
                        @Parameter(description = "Marker business ID", example = "A") @PathVariable String markerId) {
                return markerService.getMarkerWithSignedUrls(markerId)
                                .map(ResponseEntity::ok)
                                .orElse(ResponseEntity.notFound().build());
        }

        @Operation(summary = "Get Active Video", description = "Get the currently active video for a specific marker.\n\n"
                        +
        ...
        @Operation(summary = "Check Marker Exists", description = "Quick check if a marker ID exists and is active.\n\n"
                        +
        ...
        @Operation(summary = "Get Markers by Category", description = "Get all active markers for a specific category.\n\n"
                        +
        ...
        @Operation(summary = "Search Products", description = "Search markers/products by name or description.\n\n" +
                        "Returns list of matching active markers with signed URLs.")
        @GetMapping("/search")
        public ResponseEntity<List<Marker>> searchMarkers(
                        @Parameter(description = "Search query", example = "lion") @RequestParam String q) {
